...
def run_single_speed(speed, num_nodes, grid_width, grid_height, tx_range,
    ...
    # exhausts its battery (see MetricsCollector.record_round). If no node
    # actually died within the simulated window, derive a dynamic estimate
    # from the run's own observed energy-depletion rate instead of leaving
    # the CSV field empty — this rate is itself speed-dependent (higher
    # mobility => more baseline re-signaling overhead and more failed-hop
    # retransmission energy cost), so the estimate still falls as speed
    # rises even though no node literally reached zero energy this run.
    ...
    if verbose:
        print(f"  speed={speed:>4} m/s | PDR={row['pdr_percent']:>6.2f}% | "
              ...
              f"isolated={isolated_count}/{malicious_count} malicious")
