"""
engine_adapters/ateaurp_adapter.py

Adapter around engines/ateaurp (formerly Member2_ATEAURP's
run_experiments.py). Reproduces that script's own simulation loop
(NetworkManager + ATEAURPEngine.route_packet's return-value API, trust
tracking, malicious-node isolation) but returns rows instead of writing
its own CSV.
"""
...
def _run_single_speed(speed, network_manager_cls, node_cls, metrics_cls,
    ...
    isolated_count = sum(1 for n in network.nodes if n.is_isolated)
    malicious_count = sum(1 for n in network.nodes if n.is_malicious)
    detection_rate = (isolated_count / malicious_count * 100.0) if malicious_count else 0.0

    extra = {
        "malicious_nodes": malicious_count,
        "isolated_nodes": isolated_count,
        "detection_rate_percent": round(min(detection_rate, 100.0), 3),
...
def run(speeds, rounds=ROUNDS, packets_per_round=PACKETS_PER_ROUND, verbose=True,
    ...
    protocols_module = _isolated_import(ENGINE_DIR, "protocols.ateaurp")

    NetworkManager = network_module.NetworkManager
    Node = node_module.Node
    MetricsCollector = metrics_module.MetricsCollector
    write_results_csv = metrics_module.write_results_csv  # kept for parity/reuse if needed
    estimate_first_node_death_round = metrics_module.estimate_first_node_death_round
    ATEAURPEngine = protocols_module.ATEAURPEngine
    ...
    for i, speed in enumerate(speeds):
        ...
        rows.append(normalize_row("ATEAURP", speed, summary))
