"""
core/metrics.py
Metrics collector for the DRL EAURP MANET simulation.
...
class MetricsCollector:
    ...
    def network_lifetime_rounds(self):
        """
        Rounds survived before full network depletion, or the number of
        rounds actually simulated if the network never fully depleted.

        This previously returned `int(self.simulation_duration_s)`, which was
        only correct while `set_duration()` was being passed a round COUNT.
        Now that duration is a genuine number of seconds (100 ms per round,
        shared across all five engines), lifetime is tracked from
        `record_round` instead so the two units stay independent.
        """
