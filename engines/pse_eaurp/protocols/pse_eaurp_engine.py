...
class PSEEAURPEngine:
    ...
    def __init__(self, network, trust_alpha=0.7, trust_beta=0.3,
                 ...
                 contention_delay_ms=0.4, seed=42, delivery_mode="tuned"):
        # See ATEAURPEngine for the full explanation. "tuned" reproduces the
        # legacy behaviour where mobility_delivery_gate() (base constant 0.90)
        # decides delivery and the hop-by-hop result is discarded;
        # "simulated" reports whether the packet actually arrived.
    ...
    def select_next_hop(self, current_node, dest_node, visited_ids, excluded_ids=None):
        ...
        # PT_CREV revocation is supposed to make routes go AROUND a revoked
        # node ("torn down immediately and rediscovered around it"), but the
        # revocation list was never consulted during relay selection — the
        # only thing that referenced it was an abort in route_packet once a
        # packet had already been forwarded INTO a revoked node. Under the
        # legacy tuned gate that made no difference to reported PDR, so the
        # gap was invisible. Excluding revoked relays here is what actually
        # implements the documented rediscovery.
        non_revoked = [c for c in unvisited_untried
                       if c.node_id not in self.revocation_list]
        if non_revoked:
            unvisited_untried = non_revoked
    ...
    def route_packet(self, source_node, dest_node, current_round):
        ...
        def finalize(reached_destination):
        ...
        while header.hops < self.max_hops:
            ...
            if current_node.node_id in self.revocation_list:
                # Safety net only: select_next_hop now routes around revoked
                # nodes, so a packet should not normally arrive at one.
        ...
        # Hop budget exhausted without reaching the destination.
        return finalize(False)
