...
# Minimum number of packets a node must actually have been asked to relay
# (R_i) before its PFR-derived trust is considered statistically meaningful
# enough to blacklist it permanently.
#
# WHY THIS EXISTS
# ---------------
# Without it, PSE-EAURP revoked 17-21 of 50 nodes when only 5 were malicious
# — 26-36% of all BENIGN nodes were permanently blacklisted, at every speed.
#
# The mechanism: trust starts at 1.0 and updates as
#     T <- 0.7*T + 0.3*PFR,      PFR = forwarded / received
# A benign node that has been asked to relay once or twice and lost the link
# (mobility, not malice) has PFR = 0. Three rounds of that drives predicted
# trust from 1.0 to roughly 0.34, below TRUST_THRESHOLD, and
# ROUNDS_BEFORE_REVOKE fires. The node is blacklisted forever on an evidence
# base of one or two unlucky transmissions, and PT_CREV revocation is not
# reversible. Higher mobility means more link breakage, so the false-positive
# rate rises exactly when the network can least afford to lose relays.
#
# ATEAURP already guards against this in Node.evaluate_isolation, which
# requires `received > received_threshold` (default 5) before isolating a
# node. PSE-EAURP's revocation path simply never had the equivalent check.
# This restores the same convention used elsewhere in the codebase.
MIN_EVIDENCE_PACKETS = 5
...
def check_and_revoke(node_id, predicted_trust, bad_streak_tracker, revocation_list,
    ...
    evidence_count: R_i, how many packets this node has been asked to relay.
        A node with fewer than MIN_EVIDENCE_PACKETS is never revoked, since
        its PFR is not yet statistically meaningful. Passing None disables
        the guard (legacy behaviour).
    ...
    # Not enough evidence yet: don't accumulate a bad streak on noise.
    if evidence_count is not None and evidence_count < MIN_EVIDENCE_PACKETS:
        return False
