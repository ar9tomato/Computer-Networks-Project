...
class ATEAURPEngine:
    ...
    def __init__(self, network, trust_alpha=0.7, trust_beta=0.3,
                 ...
                 contention_delay_ms=0.5, seed=42, delivery_mode="tuned"):
        # delivery_mode:
        #   "tuned"     — legacy behaviour. The per-hop simulation below runs,
        #                 its outcome is DISCARDED, and delivery is decided by
        #                 mobility_delivery_gate(), whose coefficients were
        #                 hand-picked to land on ~78% / ~62% PDR targets.
        #   "simulated" — a packet counts as delivered if and only if it
        #                 actually reached the destination in the hop-by-hop
        #                 simulation. PDR then becomes a measured outcome of
        #                 routing, comparable with AODV/EAURP/MADRL-EAURP.
    ...
    def route_packet(self, source_node, dest_node, current_round):
        ...
        def finalize(reached_destination):
            if self.delivery_mode == "simulated":
                # The hop-by-hop simulation IS the answer.
                delivered = reached_destination
            else:
                # Legacy: a single end-to-end mobility gate decides the
                # reported outcome, independent of how the per-hop
                # simulation above actually played out.
        ...
        while header.hops < self.max_hops:
            ...
            if not hop_delivered:
                # Ran out of usable relays at this hop position.
